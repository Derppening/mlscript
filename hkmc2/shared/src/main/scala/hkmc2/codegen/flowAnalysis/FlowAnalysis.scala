...
object TrackableFieldSelect:
  def unapply(s: Select): Opt[Path -> (field: TermSymbol, owner: ClassSymbol)] =
    ...
    case S(sSym) if sSym.asTrm.exists(_.decl.exists(_.isInstanceOf[Param])) =>
      ...
      // * The qualifier is tracked by what it refers to, so an unchecked cast on it is transparent here: leaving it
      // * on makes every downstream `Value.SimpleRef` match fail and silently disables fusion for the selection.
