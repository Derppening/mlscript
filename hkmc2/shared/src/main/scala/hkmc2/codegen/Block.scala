      ...
      case S(ts: TermSymbol) => ts.erasedType match
        case S(ErasedType.FuncRef(paramLists, ret)) =>
          argss.sizeCompare(paramLists) match
            ...
            case 0 => ret
            // * An under-applied call yields a function type over the remaining parameter lists, the first of which
            // * carries the resource-ness of the closure the call returns.
