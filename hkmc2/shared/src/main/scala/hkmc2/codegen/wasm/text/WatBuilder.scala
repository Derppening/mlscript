...
class WatBuilder(using TraceLogger, State) extends CodeBuilder:
  ...
  private def createDefnTypes(b: Block)(using Ctx): Unit =
    b match
  ...
  def subexpression(r: codegen.Result)(using Ctx, Raise, Scope): Expr =
    r match
    case r: Lambda =>
      errExpr(
  ...
  def result(r: codegen.Result)(using Ctx, Raise, Scope): Expr =
    r match
    case Value.This(sym) =>
  ...
  def nonNestedScoped(blk: Block)(k: Block => Expr)(using Ctx, Raise, Scope): Expr =
    blk match
    case Scoped(syms, body) =>
