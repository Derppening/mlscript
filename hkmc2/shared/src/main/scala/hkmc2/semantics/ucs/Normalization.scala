            ...
            def k(ctorSym: ClassLikeSymbol, clsParams: Ls[TermSymbol])(st: Path): Block =
              ...
              def mkArgs(args: Ls[TermSymbol -> LocalVarSymbol])(using LoweringCtx): Case -> Block = args match
                ...
                case (param, arg) :: args =>
                  ...
                  // * rather than leaving each backend to re-derive it at the field read. The arm's class test proves
                  // * the narrowing, so the cast is unchecked.
                  val qual = lowering.castProvenQualifierToOwner(sr, S(param), arg.toLoc)
